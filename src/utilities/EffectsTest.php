...
use function TailwindPHP\Utilities\registerEffectsUtilities;
...
class EffectsTest extends TestCase
{
    ...
    {
        $css = $this->helper->run(['opacity-[0.33]']);
    ...
    {
        $css = $this->helper->run(['shadow-none']);
    ...
    {
        $css = $this->helper->run(['shadow-sm']);
        $this->assertStringContainsString('box-shadow:', $css);
        $this->assertStringContainsString('shadow-sm', $css);

        $css = $this->helper->run(['shadow-md']);
        $this->assertStringContainsString('box-shadow:', $css);

        $css = $this->helper->run(['shadow-lg']);
        $this->assertStringContainsString('box-shadow:', $css);

        $css = $this->helper->run(['shadow-xl']);
        $this->assertStringContainsString('box-shadow:', $css);

        $css = $this->helper->run(['shadow-2xl']);
        $this->assertStringContainsString('box-shadow:', $css);
    }
    ...
    {
        $css = $this->helper->run(['shadow-inner']);
        $this->assertStringContainsString('box-shadow:', $css);
    ...
    public function shadow_arbitrary(): void
    {
        ...
        $css = $this->helper->run(['shadow-[0_0_10px_red]']);
    ...
    {
        $css = $this->helper->run(['inset-shadow']);
        $this->assertStringContainsString('box-shadow:', $css);
    }
    ...
    {
        $css = $this->helper->run(['inset-shadow-none']);
        $this->assertStringContainsString('box-shadow: none;', $css);
    }
    ...
    {
        $css = $this->helper->run(['inset-shadow-sm']);
        $this->assertStringContainsString('box-shadow:', $css);

        $css = $this->helper->run(['inset-shadow-xs']);
        $this->assertStringContainsString('box-shadow:', $css);
    }
    ...
    {
        $css = $this->helper->run(['drop-shadow']);
    ...
    {
        $css = $this->helper->run(['drop-shadow-none']);
    ...
    {
        $css = $this->helper->run(['drop-shadow-sm']);
        $this->assertStringContainsString('filter:', $css);

        $css = $this->helper->run(['drop-shadow-md']);
        $this->assertStringContainsString('filter:', $css);

        $css = $this->helper->run(['drop-shadow-lg']);
        $this->assertStringContainsString('filter:', $css);

        $css = $this->helper->run(['drop-shadow-xl']);
        $this->assertStringContainsString('filter:', $css);

        $css = $this->helper->run(['drop-shadow-2xl']);
        $this->assertStringContainsString('filter:', $css);
    }
    ...
    {
        $css = $this->helper->run(['mix-blend-hard-light']);
        $this->assertStringContainsString('mix-blend-mode: hard-light;', $css);

        $css = $this->helper->run(['mix-blend-soft-light']);
    ...
    {
        $css = $this->helper->run(['mix-blend-difference']);
        $this->assertStringContainsString('mix-blend-mode: difference;', $css);

        $css = $this->helper->run(['mix-blend-exclusion']);
    ...
    {
        $css = $this->helper->run(['mix-blend-plus-darker']);
        $this->assertStringContainsString('mix-blend-mode: plus-darker;', $css);

        $css = $this->helper->run(['mix-blend-plus-lighter']);
    ...
    {
        $css = $this->helper->run(['bg-blend-hard-light']);
        $this->assertStringContainsString('background-blend-mode: hard-light;', $css);

        $css = $this->helper->run(['bg-blend-soft-light']);
    ...
    {
        $css = $this->helper->run(['bg-blend-difference']);
        $this->assertStringContainsString('background-blend-mode: difference;', $css);

        $css = $this->helper->run(['bg-blend-exclusion']);
