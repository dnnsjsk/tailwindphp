...
function registerSpacingUtilities(UtilityBuilder $builder): void
{
    ...
    $builder->spacingUtility('space-x', ['--space', '--spacing'], function ($value) use ($builder) {
        return [
    ...
    $builder->spacingUtility('space-y', ['--space', '--spacing'], function ($value) use ($builder) {
        return [
            decl('--tw-space-y-reverse', '0'),
            decl('margin-block-end', "calc({$value} * var(--tw-space-y-reverse))"),
            decl('margin-block-start', "calc({$value} * calc(1 - var(--tw-space-y-reverse)))"),
        ];
    }, [
        'supportsNegative' => true,
        'selector' => ':where(& > :not(:last-child))',
